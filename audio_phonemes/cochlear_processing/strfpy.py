...
def read_cochba(cochba_path=None):
    import os
    if cochba_path is None:
        # Default to same directory as this script
        cochba_path = os.path.join(os.path.dirname(__file__), 'cochba.txt')
    with open(cochba_path) as f:
        cochba = f.readlines()
    out = []
    for line in cochba:
        line = line.strip("\n").split("\t")
...
def wav2aud(x, frmlen, time_constant, fac, octave_shift, filt_type='p', use_scipy=True, return_stage=5, cochba_path=None):
    '''
    ...
    cochba_path: path to cochba.txt file (defaults to same directory as script)
    '''
    global v5
    ...
    cochba = read_cochba(cochba_path)
    L, M = cochba.shape
    L_x = len(x)
