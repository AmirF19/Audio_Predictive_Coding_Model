...
def compute_similarity_matrix(V_SL):
    """Pairwise semantic similarity between all words."""
...
def select_pairs(lexicon, sim_matrix):
    ...
    Uses all available words to maximize data.
    """
    ...
    # Same pairs (repetition) - use all available words
    ...
    for prime in available:
        ...
        candidates = np.where((sims > 0.2) & (sims < 0.9))[0]
    ...
    for prime in available:
        ...
        candidates = np.where(sims < 0.1)[0]
        for target_idx in candidates:
            target = lexicon[target_idx]
...
def plot_results(df):
    ...
    ax.set_title('N400 by Condition')
    ax.set_xticks(x)
