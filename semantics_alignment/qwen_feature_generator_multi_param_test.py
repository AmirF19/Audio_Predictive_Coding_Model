...
# Using Qwen 2.5 7B Instruct (The direct equivalent to Llama 3.1 8B)
MODEL_ID = "Qwen/Qwen2.5-7B-Instruct"

# Base output directory (Renamed for Qwen)
BASE_OUTPUT_DIR = Path("outputs_test_qwen")
...
try:
    ...
    # device_map="auto" uses 'accelerate' to place the model on the GPU automatically
...
# --- Llama 3 / Qwen System Prompt (McRae Replication) ---
# Kept exactly the same as the Llama experiment for valid comparison
SYSTEM_PROMPT = f"""
...
def run_experiment_for_temperature(temp, words):
    ...
    print(f"\n>>> STARTING QWEN EXPERIMENT: Temperature {temp}")
    print(f"    Output Folder: {current_output_dir}")

    # Define specific output files for this run
    raw_json_path = current_output_dir / "qwen_features_raw.json"
    final_json_path = current_output_dir / "qwen_model_input.json"
    final_csv_path = current_output_dir / "qwen_features_final.csv"
    ...
    pipe = pipeline(
        ...
        model=model,
        tokenizer=tokenizer,
        # --- FIX: Removed 'device' argument to allow accelerate to manage it ---
...
def main():
    print("\n==================================================")
    print("      STARTING QWEN PARAMETER SWEEP               ")
    print("==================================================")
